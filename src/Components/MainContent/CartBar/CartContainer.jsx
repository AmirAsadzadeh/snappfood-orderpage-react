...
function CartContainer() {
  ...
  const cartTotalCount = new Intl.NumberFormat("fa-IR").format(
    cartInfo.totalCounts
  );

  const cartDeleteButtonHandler = function () {
    setHasModal(true);
  };

  const confirmClearCart = function () {
    dispatch(cartActions.clearAll());
    setHasModal(false);
  };

  const cancelClearCart = function () {
    setHasModal(false);
  };
  ...
  return (
    <React.Fragment>
      <div className={classes["cart-container"]}>
        <div className={classes["cart-title-container"]}>
          <p className={classes["cart-title"]}>
            سبد خرید (
            <span className={classes["cart-total-number"]}>
              {cartTotalCount}
            </span>
            )
          </p>
          <button
            className={classes["cart-delete-button"]}
            onClick={cartDeleteButtonHandler}
          >
            <svg
